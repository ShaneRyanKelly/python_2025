...
def ai_control(state, opponent, player, balls):
    ...
    if closest_ball:
        ...
        if abs(closest_ball.rect.x - opponent.right) < 20:
            if state.gambling_mode:
                if state.opponent_stamina >= STAMINA['SLAM_COST'] and random.random() < 0.9:
                    slam_opponent = True
                elif state.opponent_stamina >= STAMINA['LOB_COST'] and random.random() < 0.9:
                    lob_opponent = True
            else:
...
def main():
    ...
    while running:
        ...
        if menu_active:
            ...
            option3 = font.render("Press 3 for 1 Player (vs Gambling AI)", True, COLORS['WHITE'])
        ...
        # Display game mode
        if state.ai_mode:
            mode_text = "GAMBLING AI MODE" if state.gambling_mode else "1P MODE"
        else:
            mode_text = "2P MODE"
