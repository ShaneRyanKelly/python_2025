...
def main():
    ...
    while running:
        for event in pygame.event.get():
            ...
            if menu_active and event.type == pygame.KEYDOWN:
                if event.key == pygame.K_1:
                    ...
                    menu_active = False
                    state.game_started = True
                elif event.key == pygame.K_0:  # Added option for no-play mode
                    state.ai_mode = True  # Both paddles controlled by AI
                    state.no_play_mode = True  # New flag for no-play mode
                    menu_active = False
                    state.game_started = True
        ...
        if menu_active:
            ...
            option0 = font.render("Press 0 for No Play (AI vs AI)", True, COLORS['WHITE'])
            ...
            screen.blit(option0, (WIDTH//2 - option0.get_width()//2, HEIGHT//2 + 100))  # Added menu option
