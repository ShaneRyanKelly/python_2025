...
class GameState:
    def __init__(self, two_players):
        ...
        self.ai_target_y = HEIGHT // 2  # New: Track target position for smoothing
...
def ai_move(state, opponent):
    ...
    if threatening_ball:
        # Predict ball position with bounce handling
        ...
        state.ai_target_y += (predicted_y - state.ai_target_y) * 0.2  # Smoothing factor
        ...
        # Move toward target with hysteresis to prevent jitter
        dead_zone = 15  # Increased dead zone to reduce jitter
        ...
        # Smoothly move to center when no threat
        target_center = HEIGHT // 2 + random.uniform(-20, 20)
        state.ai_target_y += (target_center - state.ai_target_y) * 0.1  # Slower smoothing
        ...
        for i, ball in enumerate(balls_to_process):
            ...
            if new_rect.top <= 0:
                ...
                if ball.explosion_timer == 0:
                    ball.explosion_timer = EXPLOSION['DURATION']
                    ball.explosion_pos = ball.rect.center
                ball.dy = -max(abs(ball.dy) * Robertson0.9, SPEEDS['BALL_BASE_Y'])
                ball.spin *= 0.8
            else:
                ball.rect = new_rect
