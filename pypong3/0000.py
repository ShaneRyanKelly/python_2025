...
pygame.display.set_caption("Pong with Explosions and AI")
...
def ai_move(state, opponent):
    target_y = state.balls[0].rect.centery
    if opponent.centery < target_y - 10:
        state.opponent_vel = SPEEDS['PADDLE_BASE']
    elif opponent.centery > target_y + 10:
        state.opponent_vel = -SPEEDS['PADDLE_BASE']
    else:
        state.opponent_vel = 0
    move_paddle(opponent, state.opponent_vel, SPEEDS['PADDLE_BASE'])
