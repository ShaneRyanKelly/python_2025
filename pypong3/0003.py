...
BALL = {'SIZE': 15}
SPEEDS = {
    'PADDLE_BASE': 6,    # Increased for better AI response
    'PADDLE_BOOST': 10,  # Increased for better AI dash
    'BALL_BASE_X': 5,
    'BALL_BASE_Y': 3,
    'SLAM_X': 15,
    'LOB_Y': 12
}
STAMINA = {'MAX': 100, 'DRAIN': 0.5, 'REGEN': 0.3, 'SLAM_COST': 50, 'LOB_COST': 30}  # Increased regen
...
pygame.display.set_caption("Pong with Improved AI")
...
def ai_move(state, opponent):
    # Find most threatening ball (closest to paddle horizontally and moving right)
    ...
    if threatening_ball:
        ...
        # Calculate distance and choose speed
        ...
        # Move toward predicted position with some randomness for natural feel
        reaction_threshold = 5  # Reduced for faster response
        random_offset = random.uniform(-10, 10)  # Small random variation
        target_y = predicted_y + random_offset
        
        if opponent.centery < target_y - reaction_threshold:
            state.opponent_vel = speed
        elif opponent.centery > target_y + reaction_threshold:
            state.opponent_vel = -speed
        else:
            state.opponent_vel = 0
    else:
        # Move to center with slight random variation
        center_y = HEIGHT // 2 + random.uniform(-20, 20)
        speed = SPEEDS['PADDLE_BASE']
