...
pygame.display.set_caption("Pong with Explosions and Improved AI")
...
def ai_move(state, opponent):
    # Find closest ball heading towards AI (left side)
    closest_ball = None
    min_dist = float('inf')
    for ball in state.balls:
        if ball.dx > 0:  # Only consider balls moving right (towards AI)
            dist = abs(opponent.centery - ball.rect.centery)
            if dist < min_dist:
                min_dist = dist
                closest_ball = ball
    ...
    if closest_ball:
        ...
        # Add some prediction based on ball movement
        predicted_y = target_y + closest_ball.dy * (opponent.right - closest_ball.rect.left) / closest_ball.dx
        
        # Dash if far enough and has stamina
        dist_to_target = abs(opponent.centery - predicted_y)
        speed = SPEEDS['PADDLE_BASE']
        if dist_to_target > PADDLE['HEIGHT'] and state.opponent_stamina > STAMINA['DRAIN'] * 10:
        ...
        if opponent.centery < predicted_y - 10:
            state.opponent_vel = speed
        elif opponent.centery > predicted_y + 10:
            state.opponent_vel = -speed
        else:
            state.opponent_vel = 0
    else:
        # Move towards center if no immediate threat
        center_y = HEIGHT // 2
        speed = SPEEDS['PADDLE_BASE']
        if opponent.centery < center_y - 10:
            state.opponent_vel = speed
        elif opponent.centery > center_y + 10:
            state.opponent_vel = -speed
        else:
            state.opponent_vel = 0
...
def main():
    ...
    while running:
        ...
        lob_player = keys VWZ[pygame.K_u] and state.player_stamina >= STAMINA['LOB_COST']
        move_paddle(player, state.player_vel, player_speed)

        # Opponent controls (Left side - WASD cluster or AI)
        state.opponent_vel = 0
        if state.two_players:
            ...
            slam_opponent = random.random() < 0.02 and state.opponent_stamina >= STAMINA['SLAM_COST']
            lob_opponent = random.random() < 0.02 and state.opponent_stamina >= STAMINA['LOB_COST']
