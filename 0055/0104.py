...
for idx, storm in top_5_storms.iterrows():
    ...
    ax1 = plt.gca()
    ax1.plot(storm_data['datetime'], storm_data['WSPD'], label=f"Storm {storm['storm_group']} Wind Speed")
    ax1.set_xlabel('Time')
    ax1.set_ylabel('Wind Speed (m/s)', color='tab:blue')
    ax1.tick_params(axis='y', labelcolor='tab:blue')
    
    ax2 = ax1.twinx()
    ax2.plot(storm_data['datetime'], storm_data['WVHT'], label=f"Storm {storm['storm_group']} Wave Height", linestyle='--')
    ax2.set_ylabel('Wave Height (m)', color='tab:orange')
    ax2.tick_params(axis='y', labelcolor='tab:orange')

plt.title('Comparison of Top 5 Storm Systems: Wind Speed and Wave Height')
