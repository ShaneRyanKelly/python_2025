...
df['storm_event'] = (df['is_storm'] != df['is_storm'].shift()).cumsum()
storm_events = df[df['is_storm']].groupby('storm_event').agg({
    'WSPD': ['max', 'mean'],
    'WVHT': ['max', 'mean'],
    'datetime': ['min', 'max']
}).reset_index()

storm_events.columns = ['storm_event', 'max_wspd', 'mean_wspd', 'max_wvht', 'mean_wvht', 'start_time', 'end_time']

storm_events['intensity_score'] = storm_events['max_wspd'] + storm_events['max_wvht']

top_5_storms = storm_events.sort_values('intensity_score', ascending=False).head(5)

print("\nTop 5 most significant storm events:")
print(top_5_storms[['start_time', 'end_time', 'max_wspd', 'max_wvht', 'intensity_score']])