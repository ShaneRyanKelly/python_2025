...
# Filter valid WVHT data (excluding 99.0)
df_valid = df[df['WVHT'] != 99.0]
df_valid['month'] = df_valid['datetime'].dt.month
df_valid['year'] = df_valid['datetime'].dt.year
...
# Predict missing 2024 months using historical averages
historical_avg = monthly_wvht[monthly_wvht['year'] < 2024].groupby('month')['WVHT'].mean()
missing_months = pd.DataFrame({
    'year': [2024] * 6,
    'month': [1, 2, 3, 10, 11, 12],
    'WVHT': [historical_avg.loc[m] for m in [1, 2, 3, 10, 11, 12]],
    'date': pd.to_datetime(['2024-01-01', '2024-02-01', '2024-03-01', 
                          '2024-10-01', '2024-11-01', '2024-12-01'])
})
complete_2024 = pd.concat([monthly_wvht[monthly_wvht['year'] == 2024], 
                         missing_months]).sort_values('date')

# Prepare data for visualization
# Combine original and predicted data for 2024
monthly_wvht = pd.concat([monthly_wvht[monthly_wvht['year'] != 2024], complete_2024])

# Pivot again after properly merging 2024
monthly_by_year = monthly_wvht.pivot(index='month', columns='year', values='WVHT')
monthly_by_year.columns = monthly_by_year.columns.astype(str)
