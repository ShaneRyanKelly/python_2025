...
monthly_means = df_valid.groupby(['year', 'month'])['WVHT'].mean().reset_index()
print("Monthly mean wave heights (valid data):")
print(monthly_means)

seasonal_pattern = monthly_means.groupby('month')['WVHT'].mean()
print("\nAverage wave height by month (across all years):")
print(seasonal_pattern)

print("\nMissing months to predict for 2024:")
missing_months = [10, 11, 12]
print(f"Months: {missing_months}")

print("\nHistorical monthly means for missing months:")
for month in missing_months:
    print(f"Month {month}: {seasonal_pattern.get(month, 'No historical data')}")