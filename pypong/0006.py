...
TRAIL_LENGTH = 5
SLAM_SPEED_X = 15  # Speed when slammed
SLAM_DURATION = 60  # Frames (1 second at 60 FPS)
...
slam_timer = 0  # Tracks slam duration
...
while running:
    ...
    # Slam timer management
    if slam_timer > 0:
        slam_timer -= 1
        if slam_timer == 0:
            ball_dx = BASE_SPEED_X * (1 if ball_dx > 0 else -1)  # Reset to base speed
