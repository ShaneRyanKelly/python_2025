...
pygame.display.set_caption("Pong with Spin, Stamina, and Trail")
...
RED = (255, 0, 0)
GREEN = (0, 255, 0)
TRAIL_COLOR = (150, 150, 150)  # Gray for trail
...
spin = 0
ball_trail = deque(maxlen=TRAIL_LENGTH)  # Store previous ball positions
...
while running:
    ...
    ball_trail.append((ball.centerx, ball.centery))  # Add current position to trail
    ball_dy += spin
    ball.x += ball_dx
    ball.y += ball_dy
    ...
    for i, pos in enumerate(ball_trail):
        alpha = (i + 1) / TRAIL_LENGTH  # Fade effect
        size = BALL_SIZE * (0.8 ** (TRAIL_LENGTH - i))  # Shrink effect
    ...
    STAMINA_OFFSET = 50  # Distance from paddles
