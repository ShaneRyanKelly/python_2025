...
# Global variables for AI shot decisions
slam_opponent = False
lob_opponent = False
...
class GameState:
    def __init__(self):
        ...
        self.spawn_timer = 0
        self.max_balls = 3
        self.mode = None  # None until selected, then "1p" or "2p"
...
def ai_control(state, opponent):
    if state.balls:
        # Store all active balls' positions and speeds
        ball_data = [
            {
                'rect': ball.rect,
        ...
        # Calculate priority for each ball based on distance and threat
        def ball_priority(ball_info):
            distance_x = abs(ball_info['rect'].centerx - opponent.centerx)
            distance_y = abs(ball_info['rect'].centery - opponent.centery)
            # Prioritize balls moving toward opponent (dx > 0) and closer
            threat = ball_info['speed'] * (2.0 if ball_info['dx'] > 0 else 0.5) * (1.0 - distance_x / WIDTH)
            return distance_y - threat  # Lower score = higher priority

        # Target the highest-priority ball
        target_ball_info = min(ball_data, key=ball_priority)
        target_ball = next(b for b in state.balls if b.rect == target_ball_info['rect'])
        target_y = target_ball.rect.centery
        
        # Decide movement speed (boost only if necessary)
        opponent_speed = SPEEDS['PADDLE_BASE']
        distance_to_target = abs(opponent.centery - target_y)
        # Estimate time to reach ball at base speed vs. ball's horizontal approach time
        time_to_reach = distance_to_target / SPEEDS['PADDLE_BASE']
        time_ball_approach = (WIDTH - target_ball.rect.centerx) / target_ball.dx if target_ball.dx > 0 else float('inf')
        if (state.opponent_stamina > STAMINA['DRAIN'] * 20 and 
            distance_to_target > 30 and 
            time_to_reach > time_ball_approach + 0.5):  # Add buffer to avoid late arrival
            opponent_speed = SPEEDS['PADDLE_BOOST']
            state.opponent_stamina -= STAMINA['DRAIN']
        
        # Move toward the target
        if opponent.centery < target_y - 5:
            state.opponent_vel = opponent_speed
        elif opponent.centery > target_y + 5:
            state.opponent_vel = -opponent_speed
        else:
            state.opponent_vel = 0
        
        # Decide on shot types during collision
        global slam_opponent, lob_opponent
        slam_opponent = False
        lob_opponent = False
        if target_ball.rect.colliderect(opponent):
            ball_speed = target_ball_info['speed']
            # Add randomness to shot choice for variety
            shot_chance = random.random()
            if (state.opponent_stamina >= STAMINA['SLAM_COST'] and 
                (ball_speed > SPEEDS['BALL_BASE_X'] or shot_chance < 0.4)):
                slam_opponent = True
            elif (state.opponent_stamina >= STAMINA['LOB_COST'] and 
                  (ball_speed < SPEEDS['BALL_BASE_X'] * 0.8 or shot_chance < 0.6)):
                lob_opponent = True
...
def show_menu(screen, font):
    ...
    title = font.render("Pong - Select Mode", True, COLORS['WHITE'])
    option1 = font.render("1. 1 Player (vs AI)", True, COLORS['WHITE'])
    option2 = font.render("2. 2 Players", True, COLORS['WHITE'])
...
def main():
    state = GameState()
    state.mode = show_menu(screen, font)
    running = True
    
    # Initialize global shot variables
    global slam_opponent, lob_opponent
    slam_opponent = False
    lob_opponent = False
    ...
    while running:
        ...
        if state.mode == "2p":
            ...
            if keys[pygame.K_e] and state.opponent_stamina >= STAMINA['LOB_COST']:  # E (Lob)
                ...
                lob_opponent = False
            move_paddle(opponent, state.opponent_vel, opponent_speed)
        else:  # 1-player mode, AI controls opponent
            ai_control(state, opponent)
            state.opponent_stamina = min(state.opponent_stamina + STAMINA['REGEN'], STAMINA['MAX'])
