...
# Neon color points for interpolation
NEON_COLORS = [
    (0, 255, 255),    # Cyan - slow
...
def get_ball_color(ball_dx, ball_dy):
    """Calculate neon color based on total ball velocity"""
    # Calculate total speed from both x and y components
    speed = (ball_dx ** 2 + ball_dy ** 2) ** 0.5
    max_speed = (SPEEDS['SLAM_X'] ** 2 + (SPEEDS['BALL_BASE_Y'] + SPIN['MAX']) ** 2) ** 0.5
    
    # Normalize speed to 0-1 range, then scale to number of color segments
...
def main():
    ...
    while running:
        ...
        ball_color = get_ball_color(state.ball_dx, state.ball_dy)
