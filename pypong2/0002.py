...
# Neon color ranges
NEON_COLORS = {
    'SLOW': (0, 255, 255),    # Cyan for slow speeds
    'MEDIUM': (255, 0, 255),  # Magenta for medium speeds
    'FAST': (255, 255, 0)     # Yellow for fast speeds
}
...
def get_ball_color(ball_dx):
    """Calculate neon color based on ball speed"""
    speed = abs(ball_dx)
    if speed <= SPEEDS['BALL_BASE_X'] * 1.5:
        return NEON_COLORS['SLOW']
    elif speed <= SPEEDS['SLAM_X'] * 0.75:
        return NEON_COLORS['MEDIUM']
    else:
        return NEON_COLORS['FAST']
    ...
    while running:
        ...
        for paddle, vel, is_player in [(player, state.player_vel, True), 
            ...
            if ball.colliderect(paddle):
                ...
                if slamming:  # Removed slam_timer check to allow consecutive slams
        ...
        # Trail and ball with neon colors
        ball_color = get_ball_color(state.ball_dx)
        for i, pos in enumerate(state.ball_trail):
            alpha = (i + 1) / TRAIL['LENGTH']
            size = BALL['SIZE'] * (0.8 ** (TRAIL['LENGTH'] - i))
            trail_color = tuple(int(c * alpha) for c in ball_color)  # Fade effect
