...
def main():
    ...
    while running:
        ...
        # Player controls (Right side - Numpad)
        state.player_vel = 0
        player_speed = SPEEDS['PADDLE_BASE']
        boosting_player = keys[pygame.K_KP_PLUS] and state.player_stamina > 0  # Numpad +
        ...
        if keys[pygame.K_KP8]: state.player_vel = -player_speed  # Numpad 8 (Up)
        if keys[pygame.K_KP5]: state.player_vel = player_speed   # Numpad 2 (Down)
        if keys[pygame.K_KP7] and state.player_stamina >= STAMINA['SLAM_COST']:  # Numpad 5 (Slam)
            slam_player = True
        else:
            slam_player = False
        if keys[pygame.K_KP9] and state.player_stamina >= STAMINA['LOB_COST']:  # Numpad 0 (Lob)
            lob_player = True
        else:
            lob_player = False
        move_paddle(player, state.player_vel, player_speed)

        # Opponent controls (Left side - WASD cluster)
        state.opponent_vel = 0
        opponent_speed = SPEEDS['PADDLE_BASE']
        boosting_opponent = keys[pygame.K_LSHIFT] and state.opponent_stamina > 0  # Left Shift
        ...
        if keys[pygame.K_e] and state.opponent_stamina >= STAMINA['SLAM_COST']:  # E (Slam)
            slam_opponent = True
        else:
            slam_opponent = False
        if keys[pygame.K_q] and state.opponent_stamina >= STAMINA['LOB_COST']:  # Q (Lob)
            lob_opponent = True
        else:
            lob_opponent = False
